import { type DB, schema, type Transaction } from "@repo/api/db";
...
export async function findProviderIdentity({
  ...
  if (fromIdentifier) {
    ...
    return { success: true as const, data: specificIdentity };
  }
  ...
  return { success: true as const, data: defaultIdentity };
}
