import { type DB, schema } from "@repo/api/db";
...
export async function checkAndRunKeyRotation(db: DB): Promise<Result<void>> {
  try {
    ...
    if (currentEnvVersion === currentDbVersion) {
      logger.debug(
        { currentEnvVersion, currentDbVersion },
        "Encryption version is up to date"
      );
      return { error: null, data: undefined };
    }

    logger.info({ currentDbVersion, currentEnvVersion }, "Key rotation needed");
    ...
    if (existingMigration) {
      logger.warn(
        { currentDbVersion, currentEnvVersion },
        "Key rotation migration already in progress"
      );
      return { error: null, data: undefined };
    }
...
export async function validateEncryptionKeysForStartup(
  db: DB
): Promise<Result<void>> {
  try {
    const currentEnvVersion = getCurrentKeyVersion();
    ...
    const requiredVersions = [
      ...new Set([currentEnvVersion, currentDbVersion]),
    ];
      ...
      if (keyResult.error) {
        return {
          error: createGenericError(
            `Missing required encryption key version '${version}' during startup`,
            keyResult.error
          ),
          data: null,
        };
    ...
    logger.debug(
      ...
      "Encryption key validation completed"
    );
    ...
    return {
      error: createGenericError(
        "Encryption key startup validation failed",
        error
      ),
      data: null,
    };
...
async function runKeyRotationMigration(
  ...
  try {
    ...
    logger.info(
      {
        encryptedRecordsLength: encryptedRecords.length,
        encryptedColumnRegistryLength: encryptedColumnRegistry.getAll().length,
      },
      "Migrating encrypted records"
    );
    ...
    for (let i = 0; i < encryptedRecords.length; i += batchSize) {
      ...
      logger.info(
        { migratedCount, encryptedRecordsLength: encryptedRecords.length },
        "Migrated encrypted records"
      );
    }
