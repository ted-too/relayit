...
export const templatingWorkspaceRelations = relations(
  templatingWorkspace,
  ({ one, many }) => ({
    organization: one(organization, {
      fields: [templatingWorkspace.organizationId],
      references: [organization.id],
    }),
    refs: many(templatingWorkspaceRef),
    entries: many(templatingWorkspaceEntry),
  })
);

export const templatingWorkspaceRefRelations = relations(
  templatingWorkspaceRef,
  ({ one }) => ({
    workspace: one(templatingWorkspace, {
      fields: [templatingWorkspaceRef.workspaceId],
      references: [templatingWorkspace.id],
    }),
  })
);

export const templatingWorkspaceEntryRelations = relations(
  templatingWorkspaceEntry,
  ({ one }) => ({
    workspace: one(templatingWorkspace, {
      fields: [templatingWorkspaceEntry.workspaceId],
      references: [templatingWorkspace.id],
    }),
  })
);
