...
/** Stored email From on a Campaign channel row (address + optional display name). */
export interface CampaignEmailFrom {
  address: string;
  name?: string;
  normalized: string;
}
...
export const campaignChannelFrom = pgTable(
  "campaign_channel_from",
  {
    ...
    from: jsonb("from").$type<CampaignEmailFrom>().notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at")
      .$onUpdate(() => new Date())
      .notNull(),
...
export const campaignRelations = relations(campaign, ({ one, many }) => ({
  ...
  channelFroms: many(campaignChannelFrom),
}));

export const campaignChannelFromRelations = relations(
  campaignChannelFrom,
  ({ one }) => ({
    campaign: one(campaign, {
      fields: [campaignChannelFrom.campaignId],
