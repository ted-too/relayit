...
export const dmarcReportRelations = relations(dmarcReport, ({ one, many }) => ({
  customDomain: one(customDomain, {
    fields: [dmarcReport.customDomainId],
    references: [customDomain.id],
  }),
  rows: many(dmarcReportRow),
}));
...
export const dmarcReportRowRelations = relations(dmarcReportRow, ({ one }) => ({
  report: one(dmarcReport, {
    fields: [dmarcReportRow.reportId],
    references: [dmarcReport.id],
  }),
}));
