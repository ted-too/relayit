...
export const organizationAppEnvironmentRelations = relations(
  organizationAppEnvironment,
  ({ one, many }) => ({
    organization: one(organization, {
      fields: [organizationAppEnvironment.organizationId],
...
export const organizationRelations = relations(
  ...
  ({ one, many }) => ({
    members: many(member),
    invitations: many(invitation),
    appEnvironments: many(organizationAppEnvironment),
    sandboxDomain: one(sandboxDomain, {
      fields: [organization.sandboxDomainId],
      references: [sandboxDomain.id],
    }),
    billingUser: one(user, {
      fields: [organization.billingUserId],
      references: [user.id],
    }),
  })
);
